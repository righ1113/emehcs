...
class ReplB
  def initialize(obj)
    @emehcs_obj = obj
    return unless File.exist? ConstB::READLINE_HIST_FILE

    File.open(ConstB::READLINE_HIST_FILE).readlines.each do |d|
      Readline::HISTORY.push d.chomp
    end
  end

  def prelude
    (puts 'no prelude.'; return) unless File.exist? ConstB::PRELUDE_FILE

    codes = []
    File.open(ConstB::PRELUDE_FILE, 'r') do |f|
      codes = f.read.split('|')
    end
    codes.each do |c|
      @emehcs_obj.run(c)
  ...
  def repl
    ...
    loop do
      ...
      puts "\nBye!"
      File.open(ConstB::READLINE_HIST_FILE, 'w') do |f2|
